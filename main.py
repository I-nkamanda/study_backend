...
from pathlib import (
    Path,
)  # file 시스템 경로를 객체지향적으로 다루기 위해서 (.env 파일 읽어오기 등) 사용함
import datetime as dt  # created_at 필드에 사용됨 (타임스탬프 찍기)
from contextlib import (
    asynccontextmanager,
)  # asynccontextmanager라는 데코레이터(비동기 컨텍스트 매니저 생성에 사용)를 불러옴. FastAPI의 lifespan 이벤트를 다루는 데 사용.

from fastapi import (
    FastAPI,
    HTTPException,
)  # FastAPI: 웹 애플리케이션의 핵심 클래스 / HTTPException: HTTP 오류 응답을 반환함
from pydantic import (
    BaseModel,
    EmailStr,
)  # BaseModel: 데이터 유효성 검사 / 설정 관리를 위한 기본 플래스, EmailStr: 이메일 형식의 문자열을 검증하는 데 사용됨
from dotenv import (
    load_dotenv,
)  # .env 파일에 정의된 환경 변소를 현재 환경으로 로드하는데 사용됨
...
from sqlalchemy import (
    ...
    text,
    select,
)  # String: VARCHAR와 같은 문자열 데이터 타입, text: 원시 SQL 표현식, select: SELECT 쿼리 구성에 사용됨
from sqlalchemy.orm import (
    DeclarativeBase,
    Mapped,
    mapped_column,
    sessionmaker,
)  # 불러오는 각 구성요소들은 ORM (Object Relational Mapper)를 사용, 파이썬 클래스를 DB 테이블에 매핑하는 핵심 구성 요소
from sqlalchemy.ext.asyncio import (
    create_async_engine,
    AsyncSession,
)  # SQLAlchemy를 비동기 환경에서 사용할 수 있게 해주는 클래스
import routers.chat as chat

from database.schema import lifespan
...
app = FastAPI(
    lifespan=lifespan
)  # FastAPI 애플리케이션 인스턴스를 생성하고, 위에서 정리한 lifespan 함수를 라이프사이클 관리자로 ㄷㅇ록함
...
class UserCreate(
    BaseModel
):  # Pydantic의 BaseModel을 상속받아 사용자 생성 요청 데이터의 유효성을 검사하는 schema를 정의함
